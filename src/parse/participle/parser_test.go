...
func TestParseDefaultArguments(t *testing.T) {
	...
	assert.Equal(t, "ARGS", args[2].Value.Ident.Action[0].Property.Name)
}
...
func TestParseFunctionCalls(t *testing.T) {
	...
	assert.NotNil(t, statements[0].Ident.Action[0].Call)
	assert.Equal(t, "package", statements[0].Ident.Name)
	assert.Equal(t, 0, len(statements[0].Ident.Action[0].Call.Arguments))

	assert.NotNil(t, statements[2].Ident.Action[0].Call)
	assert.Equal(t, "package", statements[2].Ident.Name)
	assert.Equal(t, 1, len(statements[2].Ident.Action[0].Call.Arguments))
	arg := statements[2].Ident.Action[0].Call.Arguments[0]
	assert.Equal(t, "default_visibility", arg.Ident.Name)
	assert.NotNil(t, arg.Ident.Action[0].Assign)
	assert.Equal(t, 1, len(arg.Ident.Action[0].Assign.List.Values))
	assert.Equal(t, "\"PUBLIC\"", arg.Ident.Action[0].Assign.List.Values[0].String)

	assert.NotNil(t, statements[3].Ident.Action[0].Call)
	assert.Equal(t, "python_library", statements[3].Ident.Name)
	assert.Equal(t, 2, len(statements[3].Ident.Action[0].Call.Arguments))
	args := statements[3].Ident.Action[0].Call.Arguments
	assert.Equal(t, "name", args[0].Ident.Name)
	assert.Equal(t, "\"lib\"", args[0].Ident.Action[0].Assign.String)
	assert.Equal(t, "srcs", args[1].Ident.Name)
	assert.NotNil(t, args[1].Ident.Action[0].Assign.List)
	assert.Equal(t, 2, len(args[1].Ident.Action[0].Assign.List.Values))
	assert.Equal(t, "\"lib1.py\"", args[1].Ident.Action[0].Assign.List.Values[0].String)
	assert.Equal(t, "\"lib2.py\"", args[1].Ident.Action[0].Assign.List.Values[1].String)

	assert.NotNil(t, statements[4].Ident.Action[0].Call)
	assert.Equal(t, "subinclude", statements[4].Ident.Name)
	assert.NotNil(t, statements[4].Ident.Action[0].Call)
	assert.Equal(t, 1, len(statements[4].Ident.Action[0].Call.Arguments))
	assert.Equal(t, "\"//build_defs:version\"", statements[4].Ident.Action[0].Call.Arguments[0].String)
}
...
func TestParseAssignments(t *testing.T) {
	...
	ass := statements[0].Ident.Action[0].Assign.Dict
	assert.NotNil(t, ass)
...
func TestForStatement(t *testing.T) {
	...
	assert.Equal(t, 2, len(statements[0].Ident.Action[0].Assign.List.Values))
...
func TestOperators(t *testing.T) {
	...
	assert.Equal(t, 2, len(statements[0].Ident.Action[0].Call.Arguments))

	arg := statements[0].Ident.Action[0].Call.Arguments[1]
	assert.Equal(t, "srcs", arg.Ident.Name)
	assert.NotNil(t, arg.Ident.Action[0].Assign.List)
	assert.Equal(t, 1, len(arg.Ident.Action[0].Assign.List.Values))
	assert.Equal(t, "\"//something:test_go\"", arg.Ident.Action[0].Assign.List.Values[0].String)
	assert.NotNil(t, arg.Ident.Action[0].Assign.Op)
	assert.Equal(t, "+", arg.Ident.Action[0].Assign.Op.Op)
	call := arg.Ident.Action[0].Assign.Op.Expr.Ident.Action[0].Call
	assert.Equal(t, "glob", arg.Ident.Action[0].Assign.Op.Expr.Ident.Name)
...
func TestIndexing(t *testing.T) {
	...
	assert.Equal(t, "x", statements[0].Ident.Name)
	assert.NotNil(t, statements[0].Ident.Action[0].Assign)
	assert.Equal(t, "\"test\"", statements[0].Ident.Action[0].Assign.String)

	assert.Equal(t, "y", statements[1].Ident.Name)
	assert.NotNil(t, statements[1].Ident.Action[0].Assign)
	assert.Equal(t, "x", statements[1].Ident.Action[0].Assign.Ident.Name)
	assert.NotNil(t, statements[1].Ident.Action[0].Assign.Slice)
	assert.Equal(t, 2, statements[1].Ident.Action[0].Assign.Slice.Start.Int)
	assert.Equal(t, "", statements[1].Ident.Action[0].Assign.Slice.Colon)
	assert.Nil(t, statements[1].Ident.Action[0].Assign.Slice.End)

	assert.Equal(t, "z", statements[2].Ident.Name)
	assert.NotNil(t, statements[2].Ident.Action[0].Assign)
	assert.Equal(t, "x", statements[2].Ident.Action[0].Assign.Ident.Name)
	assert.NotNil(t, statements[2].Ident.Action[0].Assign.Slice)
	assert.Equal(t, 1, statements[2].Ident.Action[0].Assign.Slice.Start.Int)
	assert.Equal(t, ":", statements[2].Ident.Action[0].Assign.Slice.Colon)
	assert.Equal(t, -1, statements[2].Ident.Action[0].Assign.Slice.End.Int)

	assert.Equal(t, "a", statements[3].Ident.Name)
	assert.NotNil(t, statements[3].Ident.Action[0].Assign)
	assert.Equal(t, "x", statements[3].Ident.Action[0].Assign.Ident.Name)
	assert.NotNil(t, statements[3].Ident.Action[0].Assign.Slice)
	assert.Equal(t, 2, statements[3].Ident.Action[0].Assign.Slice.Start.Int)
	assert.Equal(t, ":", statements[3].Ident.Action[0].Assign.Slice.Colon)
	assert.Nil(t, statements[3].Ident.Action[0].Assign.Slice.End)

	assert.Equal(t, "b", statements[4].Ident.Name)
	assert.NotNil(t, statements[4].Ident.Action[0].Assign)
	assert.Equal(t, "x", statements[4].Ident.Action[0].Assign.Ident.Name)
	assert.NotNil(t, statements[4].Ident.Action[0].Assign.Slice)
	assert.Nil(t, statements[4].Ident.Action[0].Assign.Slice.Start)
	assert.Equal(t, ":", statements[4].Ident.Action[0].Assign.Slice.Colon)
	assert.Equal(t, 2, statements[4].Ident.Action[0].Assign.Slice.End.Int)

	assert.Equal(t, "c", statements[5].Ident.Name)
	assert.NotNil(t, statements[5].Ident.Action[0].Assign)
	assert.Equal(t, "x", statements[5].Ident.Action[0].Assign.Ident.Name)
	assert.NotNil(t, statements[5].Ident.Action[0].Assign.Slice)
	assert.Equal(t, "y", statements[5].Ident.Action[0].Assign.Slice.Start.Ident.Name)
	assert.Equal(t, "", statements[5].Ident.Action[0].Assign.Slice.Colon)
	assert.Nil(t, statements[5].Ident.Action[0].Assign.Slice.End)
}
...
func TestInlineIf(t *testing.T) {
	...
	assert.Equal(t, "x", statements[0].Ident.Name)
	ass := statements[0].Ident.Action[0].Assign
	assert.NotNil(t, ass)
...
func TestComprehension(t *testing.T) {
	...
	assert.NotNil(t, statements[0].Ident.Action[0].Assign)
	assert.NotNil(t, statements[1].Ident.Action[0].Assign)
	assert.Equal(t, 1, len(statements[0].Ident.Action[0].Assign.List.Values))
	assert.NotNil(t, statements[0].Ident.Action[0].Assign.List.Comprehension)
	assert.NotNil(t, statements[1].Ident.Action[0].Assign.Dict.Comprehension)
}
...
func TestUnaryOp(t *testing.T) {
	...
	assert.NotNil(t, statements[0].Ident.Action[0].Assign.UnaryOp)
	assert.Equal(t, "-", statements[0].Ident.Action[0].Assign.UnaryOp.Op)
	assert.Equal(t, "len", statements[0].Ident.Action[0].Assign.UnaryOp.Expr.Ident.Name)
	assert.NotNil(t, statements[1].Ident.Action[0].Assign.UnaryOp)
	assert.Equal(t, "not", statements[1].Ident.Action[0].Assign.UnaryOp.Op)
	assert.Equal(t, "x", statements[1].Ident.Action[0].Assign.UnaryOp.Expr.Ident.Name)
}
...
func TestAugmentedAssignment(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, 1, len(statements))
	assert.NotNil(t, statements[0].Ident.Action[0].AugAssign)
}
...
func TestDestructuringAssignment(t *testing.T) {
	...
	assert.Equal(t, "x", statements[0].Ident.Name)
	assert.NotNil(t, statements[0].Ident.Action[0].Destructure)
	assert.Equal(t, "y", statements[0].Ident.Action[0].Destructure.Name)
	assert.NotNil(t, statements[0].Ident.Action[0].Destructure.Action[0].Assign)
	assert.Equal(t, "something", statements[0].Ident.Action[0].Destructure.Action[0].Assign.Ident.Name)
}
...
func TestExample5(t *testing.T) {
	_, err := NewParser().parse("src/parse/participle/test_data/example_5.build")
	assert.NoError(t, err)
}

func TestExample6(t *testing.T) {
	_, err := NewParser().parse("src/parse/participle/test_data/example_6.build")
	assert.NoError(t, err)
}
