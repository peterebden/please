...
func Please(targets []core.BuildLabel, config *core.Configuration, prettyOutput, shouldBuild, shouldTest bool) (bool, *core.BuildState) {
	...
	success := output.MonitorState(state, config.Please.NumThreads, !prettyOutput, opts.BuildFlags.KeepGoing, shouldBuild, shouldTest, shouldRun, opts.Build.ShowStatus, opts.Test.Detailed || opts.Cover.Detailed, string(opts.OutputFlags.TraceFile))
	metrics.Stop()
	build.StopWorkers()
	if c != nil {
		c.Shutdown()
