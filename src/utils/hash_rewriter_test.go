...
func TestRewriteHashes(t *testing.T) {
	...
	wd, _ := os.Getwd()
	err := core.CopyFile("src/utils/test_data/before.build", path.Join(wd, "test.build"), 0644)
	assert.NoError(t, err)
	assert.NoError(t, RewriteHashes(state.Graph, "test.build", []string{"prometheus", "cover"}))
